...
// =================================================================
export default function DippingStation({ currentUser, activeRacks, setActiveRacks }) {
  ...
  // Racks eligible for Dipping: Pickling is done, Dipping isn't yet
  const eligibleRacks = Object.keys(activeRacks).filter(
    (rackNo) => activeRacks[rackNo].picklingData && !activeRacks[rackNo].dippingData
  );
  ...
  const handleSearchRack = (targetRack) => {
    ...
    if (!record.picklingData) {
      setSelectedRack(null);
      setSearchError(`❌ Rack #${queryKey} hasn't completed Stage 02 Pickling yet.`);
      return;
    }
    if (record.dippingData) {
      setSelectedRack(null);
      setSearchError(`⚠️ Rack #${queryKey} has already completed Dipping.`);
      return;
    }
  ...
  const handleSubmitDipping = () => {
    ...
    const updatedRack = {
      ...
      dippingData: {
        ...
        zincTempC: formData.zincTempC,
        durationSecs: formData.dipDurationSecs,
    ...
    alert(`✅ Step [DIPPING] successfully recorded for Rack #${rackNo}!`);

    setSelectedRack(null);
    setInputRackNo('');
    setFormData({ zincTempC: '450', dipDurationSecs: '90', notes: '' });
  };
    ...
    <div className="w-full max-w-6xl mx-auto p-4 bg-slate-900 text-slate-100 rounded-xl shadow-2xl font-sans">
      ...
      {selectedRack && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          {/* Left Panel: Rack Progress So Far (up to, not including, Dipping) */}
            ...
            <div>
              <div className="pb-3 border-b border-slate-800 mb-4 flex justify-between items-center">
                <div>
                  ...
                  <h3 className="text-base font-bold text-white">⚡ Perform Step 03: Dipping</h3>
                </div>
              ...
              <div className="space-y-3 text-xs">
                <div>
                  <label className="block text-slate-400 mb-1">Zinc Bath Temp (°C):</label>
                  <input
                    type="number"
                    value={formData.zincTempC}
                    onChange={(e) => setFormData({ ...formData, zincTempC: e.target.value })}
                    className="w-full bg-slate-900 border border-slate-800 rounded px-3 py-2 text-white"
                  />
                </div>
                <div>
                  <label className="block text-slate-400 mb-1">Dip Duration (Secs):</label>
                  <input
                    type="number"
